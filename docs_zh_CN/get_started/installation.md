...
我们提供了不同 PyTorch 和 CUDA 版本的 mmcv-full 预编译包，可以大大简化用户安装编译过程。强烈推荐通过预编译包来安装。另外，安装完成后可以运行 [verify_installation.py](https://github.com/open-mmlab/mmcv/scripts/verify_installation.py) 脚本来验证 mmcv-full 是否安装成功。
