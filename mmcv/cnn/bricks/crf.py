...
class ConditionalRandomField(nn.Module):
    ...
    def _validate(self,
        ...
        if tags is not None:
            if emissions.shape[:2] != tags.shape:
                raise ValueError(
                    'the first two dims of emissions and tags must match, ')

        if mask is not None:
            if emissions.shape[:2] != mask.shape:
                raise ValueError(
                    'the first two dims of emissions and mask must match, '
                    f'{tuple(emissions.shape[:2])},{tuple(mask.shape)}')
    ...
    def _compute_normalizer(self, emissions: torch.Tensor,
        ...
        for i in range(1, seq_length):
            ...
            # but we're in score space, so a sum
            # becomes a log-sum-exp: for each sample,
            # entry i stores the sum of scores of
    ...
    def _viterbi_decode(self, emissions: torch.FloatTensor,
        ...
        # score is a tensor of size
        # (batch_size, num_tags) where for every batch,
        # value at column j stores the score
        # of the best tag sequence so far that ends
        # with tag j
        # history saves where the best tags
        # candidate transitioned from; this is used
        # when we trace back the best tag sequence

        # Viterbi algorithm recursive case: we
        # compute the score of the best tag sequence
        ...
        for i in range(1, seq_length):
            ...
            # Compute the score tensor of size
            # (batch_size, num_tags, num_tags) where
            # for each sample, entry at row i and
            # column j stores the score of the best
            # tag sequence so far that ends with
        ...
        for idx in range(batch_size):
            # Find the tag which maximizes the score
            # at the last timestep; this is our best tag
            # for the last timestep
            _, best_last_tag = score[idx].max(dim=0)
            best_tags = [best_last_tag.item()]

            # We trace back where the best last tag comes from,
            # append that to our best tag
