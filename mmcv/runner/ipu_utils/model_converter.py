...
class WrappedNet(torch.nn.Module):
    def __init__(self, model, inputs_tree_manager, outputs_tree_manager, modules_to_record=[]):
        super().__init__()
        self.model = model
    ...
    def forward(self, inputs_tuple):
        ...
        for _name in self.hook_features_cpu:
            # feat_in, feat_out, idx = _dic['feat_in'], _dic['feat_out'], _dic['idx']
            outputs[_name] = self.hook_features_cpu[_name]
...
class PoplarExecutorForMMCV(PoplarExecutor):
    def __init__(self, model, logger=None, training=True, modules_to_record=[], *args, **kwargs):
        ...
        self.logger = logger
        self.hook_features_ipu = {}
        model = WrappedNet(model, self.inputs_tree_manager, self.outputs_tree_manager, modules_to_record=modules_to_record) # make torch.jit.trace convert self._model
    ...
    def run_model(self, data_dict):
        ...
        for _name, _dic in self.model.hook_features_cpu:
            self.hook_features_ipu[_name] = _dic
