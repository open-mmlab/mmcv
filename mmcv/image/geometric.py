...
def _resize(img, size, dst=None, interpolation=cv2.INTER_LINEAR):
    resize_fn = _maybe_process_in_chunks(
        cv2.resize, size, dst=dst, interpolation=interpolation)
    return resize_fn(img)


def _maybe_process_in_chunks(process_fn, *args, **kwargs):
    """This wrapper function is adopted from albumentations with the following
    link https://github.com/albumentations-team/albumentations/blob/538f67d0b06
    2905e114610b7af86d2b7f62bf902/albumentations/augmentations/functional.py#L1
    54.

    Wrap OpenCV function to enable processing images with more than 4 channels.

    Limitations:
        This wrapper requires image to be the first argument and rest must be
        sent via named arguments.
    ...
    def __process_fn(img):
        ...
        if num_channels > 3:
            ...
            img = process_fn(img, *args, **kwargs)
        return img
