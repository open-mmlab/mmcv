    ...
    for (int n = 0; n < batch_size; n++) {
        ...
        for (int c = 0; c < channels; c++) {
            ...
            for (int w = 0; w < width; w++) {
                ...
                output[index_n_c + (height - 1) * width + w] = input[index_n_c + (height - 1) * width + w];
                // do top_pool
                for (int h = height - 2; h >= 0; h--) {
                    output[index_n_c + h * width + w] = std::max(output[index_n_c + (h+1) * width + w], input[index_n_c + h * width + w]);
                } // for h
            }  // for w
        } // for c
    }  // for n
    ...
    for (int n = 0; n < batch_size; n++) {
        ...
        for (int c = 0; c < channels; c++) {
            ...
            for (int h = 0; h < height; h++) {
                ...
                output[index_n_c + h * width + width - 1] = input[index_n_c + h * width + width - 1];
                // do left_pool
                for (int w = width - 2; w >= 0; w--){
                    output[index_n_c + h * width + w] = std::max(output[index_n_c + h * width + w + 1], input[index_n_c + h * width +w]);
                } // for w
            }  // for h
        } // for c
    }  // for n
