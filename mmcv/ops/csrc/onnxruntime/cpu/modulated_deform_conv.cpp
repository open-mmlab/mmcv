    ...
    float *dst)
{
    ...
    float *columns = new float[ic_per_gp * kernel_h * kernel_w * dst_h * dst_w];
...
void MMCVModulatedDeformConvKernel::Compute(OrtKernelContext *context) {
  ...
  const float *bias_data = (bias != nullptr) ?
      reinterpret_cast<const float *>(ort_.GetTensorData<float>(bias)) : nullptr;
