...
void three_interpolate_parrots(CudaContext& ctx, const SSElement& attr,
                               ...
                               OperatorBase::out_list_t& outs) {
  int b, c, m, n;
  SSAttrs(attr).get("b", b).get("c", c).get("m", m).get("n", n).done();
  ...
  three_interpolate(b, c, m, n, points_tensor, idx_tensor, weight_tensor,
                    out_tensor);
}
...
void three_interpolate_backward_parrots(CudaContext& ctx, const SSElement& attr,
                                        ...
                                        OperatorBase::out_list_t& outs) {
  int b, c, n, m;
  SSAttrs(attr).get("b", b).get("c", c).get("n", n).get("m", m).done();
  ...
  three_interpolate_backward(b, c, n, m, grad_out_tensor, idx_tensor,
