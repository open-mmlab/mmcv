...
template <typename T>
void furthest_point_sampling_with_dist_parrots(
    T& ctx, const SSElement& attr, const OperatorBase::in_list_t& ins,
    OperatorBase::out_list_t& outs) {
  int b, n, m;
  SSAttrs(attr).get("b", b).get("n", n).get("m", m).done();
  ...
  furthest_point_sampling_with_dist(b, n, m, points_tensor, temp_tensor,
                                    idx_tensor);
}
