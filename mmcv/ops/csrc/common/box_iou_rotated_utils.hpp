...
template <typename T>
HOST_DEVICE_INLINE T single_box_iou(T const *const box1_raw,
                                    T const *const box2_raw, const int offset, const int mode_flag)
{
  ...
  // The smallest point on the right corner of the two boxes - the largest point on the left corner
  // of the two boxes is the width of the intersection W, and the same is true for H
  T inter_width = std::min(box1_raw[2], box2_raw[2]) - std::max(box1_raw[0], box2_raw[0]);
  inter_width = std::max(inter_width + offset, 0.f);
  T inter_height = std::min(box1_raw[3], box2_raw[3]) - std::max(box1_raw[1], box2_raw[1]);
