...
__mlu_func__ void recursiveSumPool(T *dst, int low_dim, int high_dim, int kernel_limit) {
  for (; high_dim > 1;) {
    ...
    if (remain_s) {       
      __bang_sumpool((T *)dst, (T *)dst, low_dim, 1, remain_s, 1, remain_s, 1, 1);     
    }
    if (repeat_s) {       
      __bang_sumpool((T *)dst + (remain_s > 0 ? low_dim : 0), (T *)dst + remain_s * low_dim,
       low_dim, kernel_limit * repeat_s, 1, kernel_limit, 1, 1, kernel_limit);    
    }
