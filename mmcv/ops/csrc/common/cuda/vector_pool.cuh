...
__global__ void query_stacked_local_neighbor_idxs_cuda_kernel(
    const T *support_xyz, const int *xyz_batch_cnt, const T *new_xyz,
...
__global__ void query_three_nn_by_stacked_local_idxs_cuda_kernel(
    ...
    const int *stack_neighbor_idxs, const int *start_len, int M,
    int num_total_grids) {
  // support_xyz: (N1 + N2 ..., 3) xyz coordinates of the features
  // new_xyz: (M1 + M2 ..., 3) centers of the ball query
...
__global__ void stack_vector_pool_backward_cuda_kernel(const float *grad_new_features,
