...
void CARAFEForwardCUDAKernelLauncher(const Tensor features, const Tensor masks,
  ...
  // one warp per pixel
  at::hip::HIPGuard device_guard(features.device());
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      features.scalar_type(), "NCHW2NHWC_Feature", ([&] {
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(BatchTranspose2DCUDAKernel<scalar_t>), dim3(batch_size * dh * dw), dim3(dim3(kTileDim,kBlockRows)), 0, stream, 
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      features.scalar_type(), "CARAFELaucherForward", ([&] {
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(CARAFEForward<scalar_t>), dim3(divideUP(num_kernels, THREADS_PER_BLOCK)), dim3(THREADS_PER_BLOCK), 0, stream, 
...
void CARAFEBackwardCUDAKernelLauncher(
  ...
  at::hip::HIPGuard device_guard(top_grad.device());
  hipStream_t stream = at::hip::getCurrentHIPStream();
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      top_grad.scalar_type(), "CARAFELaucherBackward_Feature", ([&] {
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(CARAFEBackward_Feature<scalar_t>), dim3(divideUP(num_kernels, THREADS_PER_BLOCK)), dim3(THREADS_PER_BLOCK), 0, stream, num_kernels, top_diff, bottom_masks, kernel_size,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      top_grad.scalar_type(), "FeatureSum", ([&] {
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(FeatureSum<scalar_t>), dim3(divideUP(num_kernels, THREADS_PER_BLOCK)), dim3(THREADS_PER_BLOCK), 0, stream, num_kernels, bottom_diff_hs, scale_factor, channels,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      top_grad.scalar_type(), "CARAFELaucherBackward_Mask", ([&] {
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(CARAFEBackward_Mask<scalar_t>), dim3(divideUP(num_kernels, THREADS_PER_BLOCK)), dim3(THREADS_PER_BLOCK), 0, stream, num_kernels, top_diff, bottom_data, kernel_size,
                         group_size, scale_factor, channels, input_height,
                         input_width, output_height, output_width,
