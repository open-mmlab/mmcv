...
int CARAFENAIVEForwardCUDAKernelLauncher(const Tensor features,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      features.scalar_type(), "CARAFENAIVEForward", ([&] {
        hipLaunchKernelGGL(carafe_naive_forward_cuda_kernel<scalar_t>, dim3(GET_BLOCKS(output_size)), dim3(THREADS_PER_BLOCK), 0, stream, 
...
int CARAFENAIVEBackwardCUDAKernelLauncher(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      top_grad.scalar_type(), "CARAFENAIVEBackward", ([&] {
        hipLaunchKernelGGL(carafe_naive_backward_cuda_kernel<scalar_t>, dim3(GET_BLOCKS(output_size)), dim3(THREADS_PER_BLOCK), 0, stream, 
