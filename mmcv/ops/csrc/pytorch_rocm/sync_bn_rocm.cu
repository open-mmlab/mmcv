...
void SyncBNForwardMeanCUDAKernelLauncher(const Tensor input, Tensor mean) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      input.scalar_type(), "sync_bn_forward_mean_cuda_kernel", [&] {
        hipLaunchKernelGGL(sync_bn_forward_mean_cuda_kernel<scalar_t>, dim3(channels), dim3(THREADS_PER_BLOCK), 0, stream, 
...
void SyncBNForwardVarCUDAKernelLauncher(const Tensor input, const Tensor mean,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      input.scalar_type(), "sync_bn_forward_mean_cuda_kernel", [&] {
        hipLaunchKernelGGL(sync_bn_forward_var_cuda_kernel<scalar_t>, dim3(channels), dim3(THREADS_PER_BLOCK), 0, stream, 
...
void SyncBNForwardOutputCUDAKernelLauncher(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      input.scalar_type(), "sync_bn_forward_mean_cuda_kernel", [&] {
        hipLaunchKernelGGL(sync_bn_forward_output_cuda_kernel<scalar_t>, dim3(channels), dim3(THREADS_PER_BLOCK), 0, stream, 
                input.data_ptr<scalar_t>(), mean.data_ptr<float>(),
...
void SyncBNBackwardParamCUDAKernelLauncher(const Tensor grad_output,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      grad_output.scalar_type(), "sync_bn_backward_param_cuda_kernel", [&] {
        hipLaunchKernelGGL(sync_bn_backward_param_cuda_kernel<scalar_t>, dim3(channels), dim3(THREADS_PER_BLOCK), 0, stream, 
...
void SyncBNBackwardDataCUDAKernelLauncher(const Tensor grad_output,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      grad_output.scalar_type(), "sync_bn_backward_data_cuda_kernel", [&] {
        hipLaunchKernelGGL(sync_bn_backward_data_cuda_kernel<scalar_t>, dim3(GET_BLOCKS(output_size)), dim3(THREADS_PER_BLOCK), 0, stream, 
