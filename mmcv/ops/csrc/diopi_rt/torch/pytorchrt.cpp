...
DIOPI_API const char *diopiGetVersion() { return szVersion; }
...
DIOPI_API diopiError_t diopiRequireTensor(diopiContextHandle_t ctx,
  ...
  auto options =
      at::TensorOptions(device2DeviceType(dev)).dtype(dtype2scalartype(dtype));
  ctx->arrays.push_back(at::empty(atDims, options));
  *tensor = reinterpret_cast<diopiTensorHandle_t>(&(ctx->arrays.back()));
  return diopiSuccess;
}

DIOPI_API diopiError_t diopiRequireBuffer(diopiContextHandle_t ctx,
                                          diopiTensorHandle_t *tensor,
