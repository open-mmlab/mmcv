...
void ROIAlignForwardMLUKernelLauncher(Tensor input, Tensor rois, Tensor output,
  ...
  auto output_contiguous = at::empty({num_rois, channels, aligned_height, aligned_width},
  ...
  MluOpTensorDescriptor input_desc, rois_desc, argmax_y_desc, argmax_x_desc, output_desc;
  ...
  mluOpSetRoiAlignForwardDescriptor_v2(
    roialign_desc, aligned_height, aligned_width, sampling_ratio, spatial_scale,
    pool_mode, aligned);
  ...
  if (pool_mode == 0) {
    ...
    auto argmax_x_impl =
      torch_mlu::getMluTensorImpl(argmax_x_contiguous);
    auto argmax_y_impl =
      torch_mlu::getMluTensorImpl(argmax_y_contiguous);
    auto argmax_x_ptr =
      argmax_x_impl->cnnlMalloc();
    auto argmax_y_ptr =
      argmax_y_impl->cnnlMalloc();
    ...
    mluOpRoiAlignForward_v2(
        handle, roialign_desc,
        input_desc.desc(), self_ptr,
        rois_desc.desc(), rois_ptr,
        output_desc.desc(), output_ptr,
        argmax_x_desc.desc(), argmax_x_ptr,
...
void ROIAlignBackwardMLUKernelLauncher(Tensor grad, Tensor rois,
  ...
  MluOpTensorDescriptor grads_desc, rois_desc, argmax_y_desc, argmax_x_desc, grad_input_desc;
  ...
  if (pool_mode == 0) {
    ...
    mluOpRoiAlignBackward_v2(
      handle, grads_desc.desc(),
      grad_ptr, rois_desc.desc(),
      rois_ptr, argmax_y_desc.desc(),
      argmax_x_ptr, argmax_y_desc.desc(),
      argmax_y_ptr, spatial_scale, sampling_ratio,
      aligned, pool_mode,
    ...
    mluOpRoiAlignBackward_v2(
      ...
      rois_ptr, NULL,
      NULL, NULL,
      NULL, spatial_scale, sampling_ratio,
      aligned, pool_mode,
      grad_input_desc.desc(), grad_input_ptr);
  }
