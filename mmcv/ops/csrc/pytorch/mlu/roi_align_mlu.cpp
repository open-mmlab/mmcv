...
void ROIAlignForwardMLUKernelLauncher(Tensor input, Tensor rois, Tensor output,
  ...
  auto input_tmp = torch_mlu::cnnl::ops::cnnl_contiguous(input, memory_format);
  rois = torch_mlu::cnnl::ops::cnnl_contiguous(rois, c10::MemoryFormat::Contiguous);
  ...
  // generate prototxt
  if (CNNL_GEN_CASE_ON) {
    GEN_CASE_START("roialign_forward", "ROIALIGN_FORWARD");
    GEN_CASE_DATA(true, "input", input_tmp, 10, 0);
    GEN_CASE_DATA_REAL(true, "boxes", rois);
    GEN_CASE_DATA(false, "output", output_tmp, 0, 0);
    GEN_CASE_OP_PARAM_SINGLE(0, "roialign", "pooled_height", aligned_height);
    GEN_CASE_OP_PARAM_SINGLE(1, "roialign", "pooled_width", aligned_width);
    GEN_CASE_OP_PARAM_SINGLE(1, "roialign", "sampling_ratio", sampling_ratio);
    GEN_CASE_OP_PARAM_SINGLE(1, "roialign", "spatial_scale", spatial_scale);
    GEN_CASE_OP_PARAM_SINGLE(2, "roialign", "aligned", aligned);
  ...
  // get tensor impl
  auto self_impl = torch_mlu::getMluTensorImpl(input_tmp);
...
void ROIAlignBackwardMLUKernelLauncher(Tensor grad, Tensor rois,
  ...
  // generate prototxt
  if (CNNL_GEN_CASE_ON) {
    GEN_CASE_START("roi_align_backward", "ROI_ALIGN_BACKWARD");
    GEN_CASE_DATA(true, "input", grad_, 10, 0);
    GEN_CASE_DATA_REAL(true, "input", rois);
    GEN_CASE_DATA(false, "output", grad_input_, 0, 0);
    GEN_CASE_OP_PARAM_SINGLE(0, "roi_align_backward", "spatial_scale",
                             spatial_scale);
    GEN_CASE_OP_PARAM_SINGLE(1, "roi_align_backward", "sampling_ratio",
                             sampling_ratio);
    GEN_CASE_OP_PARAM_SINGLE(2, "roi_align_backward", "aligned", aligned);
    GEN_CASE_TEST_PARAM(true, true, false, 0.003, 0.003, 0);
  }
  ...
  cnrtJobType_t k_type = CNRT_FUNC_TYPE_UNION1;
  int need_core = nearestPower2(boxes_num * hi * wi);
