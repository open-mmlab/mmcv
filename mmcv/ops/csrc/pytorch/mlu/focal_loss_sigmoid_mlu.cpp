...
void SigmoidFocalLossForwardMLUKernelLauncher(Tensor input, Tensor target,
  ...
  if (CNNL_GEN_CASE_ON) {
    ...
    GEN_CASE_DATA_REAL(true, "input", input);
    GEN_CASE_DATA_REAL(true, "target", target);
    if (weight.data_ptr() != nullptr) {
      GEN_CASE_DATA_REAL(true, "weight", weight);
    }
    GEN_CASE_DATA(false, "output", output, 0, 0);
    GEN_CASE_OP_PARAM_SINGLE(0, "focal_loss_sigmoid_forward", "prefer", 0);
...
void SigmoidFocalLossBackwardMLUKernelLauncher(Tensor input, Tensor target,
  ...
  if (CNNL_GEN_CASE_ON) {
    ...
    GEN_CASE_DATA(true, "input", input, 20, -20);
    if (weight.data_ptr() != nullptr) {
      GEN_CASE_DATA_REAL(true, "target", target);
      GEN_CASE_DATA(true, "grad", output, 1, 1);
      GEN_CASE_DATA(true, "weight", weight, 1, 0);
    } else {
      GEN_CASE_DATA(true, "target", target, input.size(1), 0);
      GEN_CASE_DATA(true, "grad", output, 1, 1);
    }
    GEN_CASE_DATA(false, "output", output, 0, 0);
    GEN_CASE_OP_PARAM_SINGLE(0, "focal_loss_sigmoid_backward", "prefer", 0);
    GEN_CASE_OP_PARAM_SINGLE(1, "focal_loss_sigmoid_backward", "reduction", 0);
