...
void TINShiftForwardMLUKernelLauncher(Tensor input, Tensor shift,
                                      Tensor output) {
  // generate cnnlTinShiftForward prototxt
  if (CNNL_GEN_CASE_ON) {
    GEN_CASE_START("tin_shift", "TIN_SHIFT");
    GEN_CASE_DATA(true, "input", input, -10, 10);
    GEN_CASE_DATA(true, "shift", shift, -5, 5);
    GEN_CASE_DATA(false, "output", output, 0, 0);
    GEN_CASE_TEST_PARAM(false, false, true, 0, 0, 0);
  }
...
void TINShiftBackwardMLUKernelLauncher(Tensor grad_output, Tensor shift,
                                       Tensor grad_input) {
  // generate cnnlTinShiftBackward prototxt
  if (CNNL_GEN_CASE_ON) {
    GEN_CASE_START("tin_shift_backward", "TIN_SHIFT_BACKWARD");
    GEN_CASE_DATA(true, "grad_output", grad_output, -10, 10);
    GEN_CASE_DATA(true, "shift", shift, -5, 5);
    GEN_CASE_DATA(false, "grad_input", grad_input, 0, 0);
    GEN_CASE_TEST_PARAM(false, false, true, 0, 0, 0);
  }
