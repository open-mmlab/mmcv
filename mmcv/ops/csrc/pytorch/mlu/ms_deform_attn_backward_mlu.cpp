...
void MsDeformAttnBackwardLauncher(
  ...
  auto grad_output_dim4 = grad_output.view({batch_size, num_queries, num_heads, channels});
  // auto grad_output_dim4 = grad_output.view({batch_size, num_queries, num_heads, channels}).detach();
  ...
  mluOpMsDeformAttnBackward(
      ...
      grad_output_dim4_desc.desc(), grad_output_dim4_ptr, im2col_step,
      grad_value_desc.desc(), grad_value_ptr,
...
void ms_deform_attn_mlu_backward(
    ...
    Tensor& grad_value, Tensor& grad_sampling_loc,
    Tensor& grad_attn_weight, const int im2col_step) {
  return MsDeformAttnBackwardLauncher(
             value, spatial_shapes, level_start_index, sampling_loc,
             attn_weight, grad_output, grad_value, grad_sampling_loc,
