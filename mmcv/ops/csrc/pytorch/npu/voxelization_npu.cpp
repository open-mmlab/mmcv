...
int hard_voxelize_forward_npu(const at::Tensor &points, at::Tensor &voxels,
  ...
  at::Tensor voxel_num = at_npu::native::NPUNativeFunctions::npu_dtype_cast(voxel_num_tmp, at::kInt);

  at::Tensor voxel_size_cpu = at::from_blob(const_cast<float *>(voxel_size.data()), {3}, dtype(at::kFloat));
  at::Tensor voxel_size_npu = CalcuOpUtil::CopyTensorHostToDevice(voxel_size_cpu);

  at::Tensor coors_range_cpu = at::from_blob(const_cast<float *>(coors_range.data()), {6}, dtype(at::kFloat));
  at::Tensor coors_range_npu = CalcuOpUtil::CopyTensorHostToDevice(coors_range_cpu);
