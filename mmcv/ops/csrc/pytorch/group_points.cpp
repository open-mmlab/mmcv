...
void stack_group_points_forward_impl(int b, int c, int m, int nsample,
  ...
  DISPATCH_DEVICE_IMPL(stack_group_points_forward_impl, b, c, m, nsample,
                       features_tensor, features_batch_cnt_tensor, idx_tensor, idx_batch_cnt_tensor, out_tensor);
}
...
void stack_group_points_backward(Tensor grad_out_tensor, Tensor idx_tensor,
                           ...
                           Tensor grad_features_tensor, int b, int c, int m, int n,
                           int nsample) {
  stack_group_points_backward_impl(b, c, m, n, nsample, grad_out_tensor,
                             idx_tensor, idx_batch_cnt_tensor,features_batch_cnt_tensor,
                             grad_features_tensor);
}

void stack_group_points_forward(Tensor features_tensor, Tensor features_batch_cnt_tensor,
