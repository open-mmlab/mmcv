...
void chamfer_distance_forward(const Tensor xyz1, const Tensor xyz2,
  ...
  if (&diopiChamferDistanceMmcv) {
    diopiChamferDistanceMmcv(ch, dist1_p, dist2_p, idx1_p, idx2_p, xyz1_p, xyz2_p);
  } else {
    chamfer_distance_forward_impl(xyz1, xyz2, dist1, dist2, idx1, idx2);
  }
#else
...
void chamfer_distance_backward(const Tensor xyz1, const Tensor xyz2,
  ...
  if (&diopiChamferDistanceBackwardMmcv) {
    diopiChamferDistanceBackwardMmcv(ch, gradxyz1_p, gradxyz2_p, xyz1_p, xyz2_p,
                                 idx1_p, idx2_p, graddist1_p, graddist2_p);
  } else {
    chamfer_distance_backward_impl(xyz1, xyz2, idx1, idx2, graddist1, graddist2,
                                   gradxyz1, gradxyz2);
  }
