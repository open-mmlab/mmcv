...
// template <typename scalar_t>
// void correlation_cpp_forward_kernel(
//     const Tensor input1,
//     const Tensor input2,
//     Tensor output,
//     int kH, int kW,
...
//   const int dilatedKH = (kH - 1) * dilationH + 1;
//   const int dilatedKW = (kW - 1) * dilationW + 1;
//   //printf("batch_size:%d,iH:%d,iW:%d,patchRadH:%d,patchRadW:%d", batch_size, iH, iW, patchRadH, patchRadW);
//   const auto oH = (iH + 2 * padH - dilatedKH) / dH + 1;
//   const auto oW = (iW + 2 * padW - dilatedKW) / dW + 1;
//   auto output_temp = at::zeros({batch_size, patchH, patchW, oH, oW}, input1.options());
...
//           for (w = 0; w < oW; ++w) {
//             scalar_t tmp = 0;
//             correlate_patch<scalar_t>(input1[n],
//                             input2[n],
//                             &tmp,
//                             kH, kW,
//                             dilationH, dilationW,
...
//                             (ph - patchRadH)  * dilation_patchH,
//                             (pw - patchRadW)  * dilation_patchW);
//             output[n][ph][pw][h][w] = tmp;
//           }
//         }
//       }
//     }
//   }
//   //output = output_temp.detach();
// }
// template <typename scalar_t>
// void correlation_backward_cpu_kernel(torch::Tensor input1,
//                                      torch::Tensor input2,
...
//           {
//             scalar_t tmp = 0;
//             correlate_patch_grad<scalar_t>(input1[n], gradInput1[n],
//                                            input2[n], gradInput2[n],
//                                            &tmp,
//                                            kH, kW,
//                                            dilationH, dilationW,
//                                            -padH + h * dH,
...
//   // return {gradInput1, gradInput2};
// }
// void correlationForwardCPUKernelLaucher(const Tensor input1, const Tensor input2, Tensor output, int kH, int kW,
//                                         int patchH, int patchW,
//                                         int padH, int padW,
//                                         int dilationH, int dilationW,
//                                         int dilation_patchH, int dilation_patchW,
//                                         int dH, int dW)
// {
//   AT_DISPATCH_FLOATING_TYPES_AND_HALF(input1.scalar_type(), "correlation_cpp_forward_kernel", [&]
//                                       { correlation_cpp_forward_kernel<scalar_t>(input1, input2,
//                                                                                  output,
//                                                                                  kH, kW,
//                                                                                  patchH, patchW,
//                                                                                  padH, padW,
...
//                                                                                  dH, dW); });
// }
// void correlationBackwardCPUKernelLaucher(Tensor input1,
//                                          Tensor input2,
//                                          Tensor gradOutput,
//                                          Tensor grad_input1,
//                                          Tensor grad_input2,
...
//                                          int dH, int dW)
// {
//   AT_DISPATCH_FLOATING_TYPES_AND_HALF(input1.scalar_type(), "correlation_cpp_backward_kernel", [&]
//                                       { correlation_backward_cpu_kernel<scalar_t>(input1, input2,
//                                                                                   gradOutput,
...
void correlation_backward_cpu(Tensor input1,
                              Tensor input2,
                              Tensor gradOutput,
                              Tensor grad_input1,
                              Tensor grad_input2,
...
void correlation_forward_impl(Tensor input1, Tensor input2, Tensor output,
                              ...
                              int dilation_patchH, int dilation_patchW, int dH,
                              int dW);
void correlation_backward_impl(Tensor input1,
                               Tensor input2,
                               Tensor gradOutput,
                               Tensor grad_input1,
                               Tensor grad_input2,
