...
void ChamferDistanceForwardCUDAKernelLauncher(const Tensor xyz1, const Tensor xyz2,
      ...
      xyz1.scalar_type(), "chamfer_distance_forward_cuda_kernel", [&] {
        chamfer_distance_forward_cuda_kernel<scalar_t>
            <<<GET_BLOCKS(batch_size * n), THREADS_PER_BLOCK, 0, stream>>>(
                batch_size,
                n, xyz1.data_ptr<scalar_t>(),
                m, xyz2.data_ptr<scalar_t>(),
...
void ChamferDistanceBackwardCUDAKernelLauncher(const Tensor xyz1, const Tensor xyz2,
      ...
      [&] {
        chamfer_distance_backward_cuda_kernel<scalar_t>
            <<<GET_BLOCKS(batch_size * n), THREADS_PER_BLOCK / 2, 0, stream>>>(
                batch_size,
                m, xyz1.data_ptr<scalar_t>(),
                n, xyz2.data_ptr<scalar_t>(),
