...
#define MAX_FILTER_SIZE 32
       ...
       // Combined up/down filter buffers so that transfer can be done with one
       // copy.
...
#define g_fd (g_fbuf + MAX_FILTER_SIZE * MAX_FILTER_SIZE)
       ...
       static __global__ void setup_filters_kernel(
       ...
       // Host function to copy filters written by setup kernel into constant
       // buffer for main kernel.
       template <bool, bool>
       static cudaError_t copy_filters(cudaStream_t stream) {
         ...
         return cudaMemcpyToSymbolAsync(
             c_fbuf, src, 2 * MAX_FILTER_SIZE * MAX_FILTER_SIZE * sizeof(float),
       ...
       extern __shared__ char
           s_buf_raw[];  // When sharedKB <= 48, allocate shared memory
                         // statically inside the kernel, otherwise use the
                         // externally allocated shared memory buffer.

       template <class T, class index_t, int sharedKB, bool signWrite,
                 bool signRead, int filterMode, int up, int fuSize, int down,
                 int fdSize, int tileOutW, int tileOutH, int threadsPerBlock,
                 bool enableXrep, bool enableWriteSkip>
       static __global__ void filtered_lrelu_kernel(
           filtered_lrelu_kernel_params p) {
         ...
         static_assert(fdSize % down == 0,
                       "downsampling filter size must be divisible with "
                       "downsampling factor");
         ...
         static_assert(!(down == 4 &&
                         (filterMode == MODE_FUFD || filterMode == MODE_SUFD)),
         ...
         const int tileUpW =
             (tileOutW * down + (fdSize - 1) - (down - 1) + 3) &
             ~3;  // Upsampled tile width, rounded up to multiple of 4.
         const int tileUpH = tileOutH * down + (fdSize - 1) -
                             (down - 1);  // Upsampled tile height.
         const int tileInW =
         ...
         if (sharedKB <= 48) {
           ...
           __shared__ scalar_t
               s_buf0_st[(sharedKB > 48)
                             ? (1 << 24)
                             : (s_buf0_size +
                                s_buf1_size)];  // Prevent launching if this
                                                // isn't optimized away when
                                                // unused.
           s_buf0 = s_buf0_st;
         ...
         scalar_t* s_tileIn;     // Input tile:                      [relInX *
                                 // tileInH + relInY]
         scalar_t* s_tileUpX;    // After horizontal upsampling:     [relInY *
                                 // tileUpW + relUpX]
         scalar_t* s_tileUpXY;   // After upsampling:                [relUpY *
                                 // tileUpW + relUpX]
         scalar_t* s_tileDownX;  // After horizontal downsampling:   [relUpY *
                                 // tileOutW
                                 // + relOutX]
...
#pragma unroll 1
         for (int tileIdx = 0;
              !enableXrep ||
              (tileIdx <
               MIN(p.tilesXrep, p.tilesXdim - p.tilesXrep * blockIdx.y));
              tileIdx++) {
           // Locate output tile.
           int tileX =
               enableXrep ? blockIdx.y * p.tilesXrep + tileIdx : blockIdx.x;
           ...
           // Extra sync if input and output buffers are the same and we are not
           // on first tile.
           if (enableXrep && tileIdx > 0 &&
               (filterMode == MODE_FUSD ||
                (filterMode == MODE_SUFD && !downInline) ||
...
#pragma unroll
           for (int loop = 0; loop < loopCountIN; loop++) {
             ...
             bool skip =
                 (loop == loopCountIN - 1) && (idx >= tileInW * tileInH);
...
#pragma unroll
             ...
             if (up == 2) {
               ...
               __syncthreads();
               int minY =
                   tileOutY ? (tileOutY - tileOutH) * down + tileUpH + p.sOfs.y
                            : 0;  // Skip already written signs.
...
#pragma unroll
               for (int step = 0; step < fdSize; step++)
                 v +=
                     s_tileDownX[src0 + step * tileOutW] * (scalar_t)c_fd[step];
...
#pragma unroll
                 ...
                 if ((uint32_t)outY < p.yShape.y) {
                   ...
                   if (outX + 1 < p.yShape.x)
                     *((T*)((char*)p.y + ofs +
                            get_stride<index_t>(p.yStride.x))) = (T)v.y;
                 }
               }
               ...
               for (int idx = threadIdx.x; idx < tileOutW * tileOutH;
                 ...
                 fast_div_mod<tileOutW>(relOutX0, relOutY0, idx);
                 scalar_t v =
                     s_tileUpXY[idx] * (scalar_t)c_fd[0];  // 1x1 filter.
       ...
       //------------------------------------------------------------------------
       // Compute activation function and signs for upsampled data tensor,
       // modifying data tensor in-place. Used for accelerating the generic
       // variant. Sign tensor is known to be contiguous, and p.x and p.s have
       // the same z, w dimensions. 64-bit indexing is always used.
    ...
    if ((p.fuShape.y == 0 && (MODE == MODE_SUSD || MODE == MODE_SUFD)) ||      \
      ...
      if ((p.fdShape.y == 0 && (MODE == MODE_SUSD || MODE == MODE_FUSD)) ||    \
        ...
        if (p.up == U && p.fuShape.x <= FU && p.fuShape.y <= FU &&             \
         ...
         // Small filters must be listed before large filters, otherwise the
         // kernel for larger filter will always match first. Kernels that use
         // more shared memory must be listed before those that use less, for
         // the same reason.

         CASE(/*sharedKB*/ 48, /*up,fu*/ 1, 1, /*down,fd*/ 1, 1,
              /*mode*/ MODE_FUFD,
              /*tw,th,warps,xrep,wskip*/ 64, 178, 32, 0, 0)  // 1t-upf1-downf1
         CASE(/*sharedKB*/ 48, /*up,fu*/ 2, 8, /*down,fd*/ 1, 1,
              /*mode*/ MODE_SUFD,
              /*tw,th,warps,xrep,wskip*/ 152, 95, 16, 0, 0)  // 4t-ups2-downf1
         CASE(/*sharedKB*/ 48, /*up,fu*/ 1, 1, /*down,fd*/ 2, 8,
              /*mode*/ MODE_FUSD,
              /*tw,th,warps,xrep,wskip*/ 56, 22, 16, 0, 0)  // 4t-upf1-downs2
         CASE(/*sharedKB*/ 48, /*up,fu*/ 2, 8, /*down,fd*/ 2, 8,
              /*mode*/ MODE_SUSD,
         ...
         CASE(/*sharedKB*/ 48, /*up,fu*/ 4, 32, /*down,fd*/ 2, 16,
              /*mode*/ MODE_SUSD,
              /*tw,th,warps,xrep,wskip*/ 32, 46, 24, 5, 0)  // 8t-ups4-downs2
         CASE(/*sharedKB*/ 48, /*up,fu*/ 4, 32, /*down,fd*/ 2, 16,
              /*mode*/ MODE_SUFD,
              /*tw,th,warps,xrep,wskip*/ 32, 50, 16, 0, 0)  // 8t-ups4-downf2
         CASE(/*sharedKB*/ 96, /*up,fu*/ 2, 16, /*down,fd*/ 4, 32,
              /*mode*/ MODE_SUSD,
              /*tw,th,warps,xrep,wskip*/ 24, 24, 32, 12,
              1)  // 8t-ups2-downs4 96kB
         CASE(/*sharedKB*/ 48, /*up,fu*/ 2, 16, /*down,fd*/ 4, 32,
              /*mode*/ MODE_SUSD,
              /*tw,th,warps,xrep,wskip*/ 16, 13, 16, 10, 1)  // 8t-ups2-downs4
         CASE(/*sharedKB*/ 96, /*up,fu*/ 2, 16, /*down,fd*/ 4, 32,
              /*mode*/ MODE_FUSD,
              /*tw,th,warps,xrep,wskip*/ 25, 28, 28, 4,
              0)  // 8t-upf2-downs4 96kB
         CASE(/*sharedKB*/ 48, /*up,fu*/ 2, 16, /*down,fd*/ 4, 32,
              /*mode*/ MODE_FUSD,
...
#undef CASE
       ...
       // Template/kernel specializations for no signs mode (no gradients
       // required).
       ...
       template void* choose_filtered_lrelu_act_kernel<c10::Half, false, false>(
           void);
       template void* choose_filtered_lrelu_act_kernel<float, false, false>(
           void);
       template void* choose_filtered_lrelu_act_kernel<double, false, false>(
           void);
       ...
       // Activation/signs only for generic variant. 64-bit indexing.
       template void* choose_filtered_lrelu_act_kernel<c10::Half, false, true>(
           void);
       template void* choose_filtered_lrelu_act_kernel<float, false, true>(
           void);
       template void* choose_filtered_lrelu_act_kernel<double, false, true>(
           void);
       ...
       // Activation/signs only for generic variant. 64-bit indexing.
       template void* choose_filtered_lrelu_act_kernel<c10::Half, true, false>(
           void);
       template void* choose_filtered_lrelu_act_kernel<float, true, false>(
           void);
       template void* choose_filtered_lrelu_act_kernel<double, true, false>(
           void);
       ...
       std::tuple<torch::Tensor, torch::Tensor, int> filtered_lrelu_op(
           ...
           torch::Tensor si, int up, int down, int px0, int px1, int py0,
           int py1, int sx, int sy, float gain, float slope, float clamp,
           bool flip_filters, bool writeSigns) {
         // Set CUDA device.
         ...
         TORCH_CHECK(b.dtype() == x.dtype(),
                     "x and b must have the same dtype");
         ...
         AT_CUDA_CHECK(cudaDeviceGetAttribute(
             &maxSharedBytes, cudaDevAttrMaxSharedMemoryPerBlockOptin,
             x.device().index()));
         ...
         p.fdShape =
             make_int2((int)fd.size(-1), fd.dim() == 2 ? (int)fd.size(0) : 0);
         filtered_lrelu_kernel_spec test_spec =
             choose_filtered_lrelu_kernel<float, int32_t, false, false>(
                 p, sharedKB);
         if (!test_spec.exec) {
           // No kernel found - return empty tensors and indicate missing kernel
         ...
         TORCH_CHECK(cw > fdt_w && ch > fdt_h,
                     "upsampled buffer must be at least the size of "
                     "downsampling filter");
         TORCH_CHECK(cw <= INT_MAX && ch <= INT_MAX,
                     "upsampled buffer is too large");
         ...
         torch::Tensor y = torch::empty({x.size(0), x.size(1), yh, yw},
         ...
         if (writeSigns) {
           sw_active =
               yw * down - (down - 1) + fdt_w;  // Active width in elements.
           int64_t sh =
               yh * down - (down - 1) + fdt_h;  // Height = active height.
           int64_t sw =
               (sw_active + 15) & ~15;  // Width  = active width in elements,
                                        // rounded up to multiple of 16.
           TORCH_CHECK(sh <= INT_MAX && (sw >> 2) <= INT_MAX,
                       "signs is too large");
         ...
         p.xShape = make_int4((int)x.size(3), (int)x.size(2), (int)x.size(1),
                              (int)x.size(0));
         p.yShape = make_int4((int)y.size(3), (int)y.size(2), (int)y.size(1),
                              (int)y.size(0));
             ...
             x.scalar_type(), "filtered_lrelu_cuda", [&] {
               if constexpr (sizeof(scalar_t) <=
                             4)  // Exclude doubles. constexpr prevents template
                                 // instantiation.
               {
                 // Choose kernel based on index type, datatype and sign
                 // read/write modes.
                 if (!index64b && writeSigns && !readSigns)
                   spec = choose_filtered_lrelu_kernel<scalar_t, int32_t, true,
                                                       false>(p, sharedKB);
                 else if (!index64b && !writeSigns && readSigns)
                   spec = choose_filtered_lrelu_kernel<scalar_t, int32_t, false,
                                                       true>(p, sharedKB);
                 else if (!index64b && !writeSigns && !readSigns)
                   spec = choose_filtered_lrelu_kernel<scalar_t, int32_t, false,
                                                       false>(p, sharedKB);
                 else if (index64b && writeSigns && !readSigns)
                   spec = choose_filtered_lrelu_kernel<scalar_t, int64_t, true,
                                                       false>(p, sharedKB);
                 else if (index64b && !writeSigns && readSigns)
                   spec = choose_filtered_lrelu_kernel<scalar_t, int64_t, false,
                                                       true>(p, sharedKB);
                 else if (index64b && !writeSigns && !readSigns)
                   spec = choose_filtered_lrelu_kernel<scalar_t, int64_t, false,
                                                       false>(p, sharedKB);
         ...
         TORCH_CHECK(
             spec.exec,
             "internal error - CUDA kernel not found")  // This should not
                                                        // happen because we
                                                        // tested earlier that
                                                        // kernel exists.
         ...
         AT_CUDA_CHECK(cudaFuncSetSharedMemConfig(
             spec.exec, cudaSharedMemBankSizeFourByte));
              ...
              zofs += maxSubGz)  // Do multiple launches if gz is too big.
         {
           ...
           AT_CUDA_CHECK(cudaLaunchKernel(spec.exec, dim3(gx, gy, subGz), bx,
                                          args, spec.dynamicSharedKB << 10,
                                          at::cuda::getCurrentCUDAStream()));
         }
       ...
       torch::Tensor filtered_lrelu_act_op(torch::Tensor x, torch::Tensor si,
         ...
         AT_DISPATCH_FLOATING_TYPES_AND_HALF(
             x.scalar_type(), "filtered_lrelu_act_cuda", [&] {
               if (writeSigns)
                 func =
                     choose_filtered_lrelu_act_kernel<scalar_t, true, false>();
               else if (readSigns)
                 func =
                     choose_filtered_lrelu_act_kernel<scalar_t, false, true>();
               else
                 func =
                     choose_filtered_lrelu_act_kernel<scalar_t, false, false>();
             });
         ...
         uint32_t gz = p.xShape.z *
                       p.xShape.w;  // Same as in p.sShape if signs are in use.
         gx = (gx - 1) / bx + 1;

         // Make sure grid y and z dimensions are within CUDA launch limits.
         // Kernel loops internally to do the rest.
