...
void StackThreeInterpolateForwardCUDAKernelLauncher(const Tensor points,
  ...
  // stack_three_interpolate_forward_cuda_kernel<<<blocks, threads>>>(N,
  // channels,points.data_ptr<float>(), idx.data_ptr<int>(),
  //                 weight.data_ptr<float>(), out.data_ptr<float>());
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      points.scalar_type(), "stack_three_interpolate_forward_cuda_kernel", [&] {
        stack_three_interpolate_forward_cuda_kernel<scalar_t>
            <<<blocks, threads>>>(
                N, channels, points.data_ptr<scalar_t>(), idx.data_ptr<int>(),
...
void StackThreeInterpolateBackwardCUDAKernelLauncher(const Tensor grad_out,
  ...
  //                     N, channels, grad_out.data_ptr<float>(),
  //                     idx.data_ptr<int>(),
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      ...
      [&] {
        stack_three_interpolate_backward_cuda_kernel<scalar_t>
            <<<blocks, threads>>>(
