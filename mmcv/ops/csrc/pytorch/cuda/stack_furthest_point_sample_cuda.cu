...
void StackFurthestPointSamplingForwardCUDALauncher(
  ...
  stack_farthest_point_sample_cuda_kernel<1024><<<batch_size, 1024>>>(
      batch_size, N, points_tensor.data_ptr<float>(),
      temp_tensor.data_ptr<float>(), xyz_batch_cnt_tensor.data_ptr<int>(),
      idx_tensor.data_ptr<int>(), num_sampled_points_tensor.data_ptr<int>());
