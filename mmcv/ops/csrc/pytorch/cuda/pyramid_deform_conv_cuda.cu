...
#include "pyramid_deform_conv_cuda_kernel.cuh"
...
void pyramid_deformable_im2col_cuda(Tensor data_im, Tensor data_offset,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      data_im.scalar_type(), "pyramid_deformable_im2col_cuda_kernel", ([&] {
        const scalar_t *data_im_ = data_im.data_ptr<scalar_t>();
        const scalar_t *data_offset_ = data_offset.data_ptr<scalar_t>();
        scalar_t *data_col_ = data_col.data_ptr<scalar_t>();

        pyramid_deformable_im2col_cuda_kernel<<<GET_BLOCKS(num_kernels),
                                       THREADS_PER_BLOCK, 0,
                                       at::cuda::getCurrentCUDAStream()>>>(
            num_kernels, data_im_, data_offset_, height, width, ksize_h,
            ksize_w, pad_h, pad_w, stride_h, stride_w, dilation_h, dilation_w, scale_h, scale_w,
...
void pyramid_deformable_col2im_cuda(Tensor data_col, Tensor data_offset,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      data_col.scalar_type(), "pyramid_deformable_col2im_cuda_kernel", ([&] {
        const scalar_t *data_col_ = data_col.data_ptr<scalar_t>();
        const scalar_t *data_offset_ = data_offset.data_ptr<scalar_t>();
        scalar_t *grad_im_ = grad_im.data_ptr<scalar_t>();

        pyramid_deformable_col2im_cuda_kernel<<<GET_BLOCKS(num_kernels),
                                       THREADS_PER_BLOCK, 0,
                                       at::cuda::getCurrentCUDAStream()>>>(
            num_kernels, data_col_, data_offset_, channels, height, width,
            ksize_h, ksize_w, pad_h, pad_w, stride_h, stride_w, dilation_h,
            dilation_w, scale_h, scale_w, channel_per_deformable_group, parallel_imgs,
...
void pyramid_deformable_col2im_coord_cuda(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      data_col.scalar_type(), "pyramid_deformable_col2im_coord_cuda_kernel", ([&] {
        const scalar_t *data_col_ = data_col.data_ptr<scalar_t>();
        const scalar_t *data_im_ = data_im.data_ptr<scalar_t>();
        const scalar_t *data_offset_ = data_offset.data_ptr<scalar_t>();
        scalar_t *grad_offset_ = grad_offset.data_ptr<scalar_t>();

        pyramid_deformable_col2im_coord_cuda_kernel<<<
            ...
            dilation_h, dilation_w,scale_h, scale_w, channel_per_deformable_group, parallel_imgs,
