...
//   const int *features_batch_cnt_ptr = features_batch_cnt_tensor.data_ptr<int>();
//   const int *idx_batch_cnt_ptr = idx_batch_cnt_tensor.data_ptr<int>();
//   float *out_ptr = out_tensor.data_ptr<scalar_t>();
      ...
      features_tensor.scalar_type(), "stack_group_points_forward_cuda_kernel", [&] {
        stack_group_points_forward_cuda_kernel<scalar_t>
            <<<blocks, threads, 0, stream>>>(
                b, c, m, nsample, features_tensor.data_ptr<scalar_t>(), idx_tensor.data_ptr<int>(),
                features_batch_cnt_tensor.data_ptr<int>(), idx_batch_cnt_tensor.data_ptr<int>(), out_tensor.data_ptr<scalar_t>());
      });
...
//   const int *idx_batch_cnt_ptr = idx_batch_cnt_tensor.data_ptr<int>();
//   const int *features_batch_cnt_ptr = features_batch_cnt_tensor.data_ptr<int>();
//   float *grad_features_ptr = grad_features_tensor.data_ptr<float>();


  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      grad_features_tensor.scalar_type(), "stack_group_points_backward_cuda_kernel", [&] {
        stack_group_points_backward_cuda_kernel<scalar_t>
            <<<blocks, threads, 0, stream>>>(
                b, c, m, n, nsample, grad_out_tensor.data_ptr<scalar_t>(),
                idx_tensor.data_ptr<int>(), idx_batch_cnt_tensor.data_ptr<int>(), features_batch_cnt_tensor.data_ptr<int>(),
