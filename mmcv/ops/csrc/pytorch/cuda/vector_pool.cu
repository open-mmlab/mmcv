...
void StackQueryLocalNeighborIdxsCUDAKernelLauncher(const Tensor support_xyz_tensor, const Tensor xyz_batch_cnt_tensor,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      support_xyz_tensor.scalar_type(), "query_stacked_local_neighbor_idxs_cuda_kernel", [&] {
        query_stacked_local_neighbor_idxs_cuda_kernel<scalar_t>
            <<<blocks, threads, 0, stream>>>(
                        support_xyz_tensor.data_ptr<scalar_t>(), xyz_batch_cnt_tensor.data_ptr<int>(), new_xyz_tensor.data_ptr<scalar_t>(),
                          new_xyz_batch_cnt_tensor.data_ptr<int>(), stack_neighbor_idxs_tensor.data_ptr<int>(), start_len_tensor.data_ptr<int>(),
                           cumsum_tensor.data_ptr<int>(), avg_length_of_neighbor_idxs,
...
void StackQueryThreeNNLocalIdxsCUDAKernelLauncher(const Tensor support_xyz_tensor,
    ...
    const int M, const int num_total_grids){
          ...
          AT_DISPATCH_FLOATING_TYPES_AND_HALF(
              support_xyz_tensor.scalar_type(), "query_three_nn_by_stacked_local_idxs_cuda_kernel", [&] {
                query_three_nn_by_stacked_local_idxs_cuda_kernel<scalar_t>
                    <<<blocks, threads, 0, stream>>>(
                        support_xyz_tensor.data_ptr<scalar_t>(), new_xyz_tensor.data_ptr<scalar_t>(), new_xyz_grid_centers_tensor.data_ptr<scalar_t>(),
                      new_xyz_grid_idxs_tensor.data_ptr<int>(), new_xyz_grid_dist2_tensor.data_ptr<scalar_t>(), stack_neighbor_idxs_tensor.data_ptr<int>(),
