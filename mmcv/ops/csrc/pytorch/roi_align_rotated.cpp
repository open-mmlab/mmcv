...
void roi_align_rotated_forward_cuda(Tensor features, Tensor rois,
  ...
  ROIAlignRotatedForwardCUDAKernelLauncher(features, rois, spatial_scale,
                                           sample_ratio, aligned, clockwise, num_channels,
...
void roi_align_rotated_forward_cpu(Tensor input, Tensor rois, Tensor output,
                           ...
                           int aligned_width, float spatial_scale,
                           int sampling_ratio, bool aligned, bool clockwise) {
  ROIAlignRotatedForwardCPULauncher(input, rois, output,
                             aligned_height, aligned_width, spatial_scale,
                             sampling_ratio, aligned, clockwise);
}

void roi_align_rotated_backward_cpu(Tensor top_grad, Tensor rois, Tensor bottom_grad,
                            int aligned_height, int aligned_width,
                            float spatial_scale, int sampling_ratio,
                            bool aligned, bool clockwise) {
  // Number of ROIs
  ...
  ROIAlignRotatedBackwardCPULauncher(top_grad, rois, bottom_grad,
                              aligned_height, aligned_width, spatial_scale,
                              sampling_ratio, aligned, clockwise);
}
...
void roi_align_rotated_forward(Tensor input, Tensor rois, Tensor output,
  ...
  if (input.device().is_cuda()) {
    ...
    roi_align_rotated_forward_cpu(input, rois, output,
                          aligned_height, aligned_width, spatial_scale,
                          sampling_ratio, aligned, clockwise);
  }
}
...
void roi_align_rotated_backward(Tensor top_grad, Tensor rois,
  ...
  if (top_grad.device().is_cuda()) {
    ...
    roi_align_rotated_backward_cuda(top_grad, rois, bottom_grad,
                                    aligned_height, aligned_width, spatial_scale,
                                    sampling_ratio, aligned, clockwise);
#else
    AT_ERROR("RoIAlignRotated is not compiled with GPU support");
    ...
    roi_align_rotated_backward_cpu(top_grad, rois, bottom_grad,
                           aligned_height, aligned_width, spatial_scale,
                           sampling_ratio, aligned, clockwise);
  }
}
