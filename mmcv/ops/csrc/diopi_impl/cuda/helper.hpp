...
#define ATEN_NOT_SUPPORTED()                                                   \
  LOG_LINE_INFO()                                                              \
  logError("NotImplementError: function ", __FUNCTION__,                       \
           " is not implemented for torch version ", TORCH_VERSION);
...
void updateATen2Tensor(diopiContextHandle_t ctx, const at::Tensor &atOut,
...
template <typename TupleT, std::size_t N> struct UpdateTupleATen {
...
template <typename TupleT> struct UpdateTupleATen<TupleT, 1> {
  static void update(diopiContextHandle_t ctx, TupleT &atOuts,
...
void updateATen2Tensor(diopiContextHandle_t ctx, TupleT &atOuts,
                       diopi_tensor_list &outs) {
  constexpr size_t tupleSize = std::tuple_size<TupleT>::value;
  UpdateTupleATen<TupleT, tupleSize>::update(ctx, atOuts, outs);
}

void updateATen2Tensor(diopiContextHandle_t ctx,
                       std::vector<at::Tensor> &atOuts,
                       diopi_tensor_list &outs) {
...
void invokeATenFuncRet(diopiContextHandle_t ctx, Func func,
                       diopiTensorHandle_t out, Args &&... args) {
...
template <typename Func, typename... Args>
void invokeATenFuncRet(diopiContextHandle_t ctx, Func func,
                       diopi_tensor_list &outs, Args &&... args) {
...
void buildDiopiTensor(diopiContextHandle_t ctx, at::Tensor &input,
                      diopiTensorHandle_t *out) {
...
getRoundingMode(diopiRoundMode_t rounding_mode) {
