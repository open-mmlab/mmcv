...
class RoIAlignRotatedFunction(Function):
    ...
    def forward(ctx, features, rois, out_size, spatial_scale, sample_num=0):
        ...
        if features.is_cuda:
            ext_module.roi_align_rotated_forward(features, rois, output,
                                    pooled_height=out_h, pooled_width=out_w,
                                    spatial_scale=spatial_scale, sample_num=sample_num)
        else:
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            grad_input = rois.new_zeros(batch_size, num_channels, data_height,
                                        data_width)
            ext_module.roi_align_rotated_backward(grad_output.contiguous(), rois, grad_input,
                                    pooled_height=out_h, pooled_width=out_w,
                                    spatial_scale=spatial_scale, sample_num=sample_num
                                    )
