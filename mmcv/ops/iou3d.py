...
def _xyxyr2xyzwlhr(boxes):
    """Convert [x1, y1, x2, y2, heading] box to [x, y, 0, dx, dy, 1, heading]
    box.
    ...
    return torch.stack(
        ((boxes[:, 0] + boxes[:, 2]) / 2, (boxes[:, 1] + boxes[:, 3]) / 2,
         torch.zeros_like(boxes[:, 0]), boxes[:, 2] - boxes[:, 0],
         boxes[:, 3] - boxes[:, 1], torch.ones_like(boxes[:, 0]), boxes[:, 4]),
        dim=-1)
...
def boxes_iou_bev(boxes_a, boxes_b):
    ...
    ans_iou = boxes_a.new_zeros(
        torch.Size((boxes_a.shape[0], boxes_b.shape[0])))
    boxes_a = _xyxyr2xyzwlhr(boxes_a)
    boxes_b = _xyxyr2xyzwlhr(boxes_b)
...
def nms_bev(boxes, scores, thresh, pre_max_size=None, post_max_size=None):
    ...
    boxes = _xyxyr2xyzwlhr(boxes)[order].contiguous()
