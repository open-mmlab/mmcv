...
void flowWarp(double* img, double* flow1, double* out, const int height, const int width, const int channels, const int ignoreLabel=0, const int interpolateMode=0){
    ...
    for(int h=0; h<height; h++){
        for(int w=0; w<width; w++){
            ...
            if(interpolateMode==0)
                BilinearInterpolate(img, width, height, channels, x, y, out+offset_img); 
...
void BilinearInterpolate(const double* img, int width, int height, int channels, double x, double y, double* out){
    ...
    for(m=0; m<=1; m++)
        for(n=0; n<=1; n++){
            ...
            for(l=0; l<channels; l++)
                out[l] += img[offset_img+l] * s;
        }
}

void NNInterpolate(const double* img, int width, int height, int channels, double x, double y, double* out){
    int xx, yy, m, n, u, v, offset, offset_img, l;
    xx = x;
    yy = y;
