...
__global__ void deform_roi_pool_forward_cuda_kernel(
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    int roi_bin_grid_h =
        (sampling_ratio > 0) ? sampling_ratio : static_cast<int>(ceil(roi_height / pooled_height));
    int roi_bin_grid_w =
        ...
        for (int ix = 0; ix < roi_bin_grid_w; ix++) {
            ...
            if (x_low >= 0 && x_high >= 0 && y_low >= 0 && y_high >= 0) {
                ...
                if (offset != NULL) {
                    ...
                    T ogx = gamma * roi_width * grad_output_this_bin * (input_11 * (y - y_low) +
                        input_10 * (y_high - y) + input_01 * (y_low - y) + input_00 * (y - y_high));
                    T ogy = gamma * roi_height * grad_output_this_bin * (input_11 * (x - x_low) +
                        input_01 * (x_high - x) + input_10 * (x_low - x) + input_00 * (x - x_high));
