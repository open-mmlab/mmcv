...
void deformable_im2col(
    DArrayLite data_im, DArrayLite data_offset, const int channels,
    ...
    PARROTS_DISPATCH_FLOATING_TYPES_AND_HALF(data_im.elemType().prim(), ([&] {
        deformable_im2col_gpu_kernel<scalar_t><<<GET_BLOCKS(num_kernels), THREADS_PER_BLOCK, 0, stream>>>(
            ...
            height_col, width_col, data_col.ptr<scalar_t>());
        }));
...
void deformable_col2im(
    DArrayLite data_col, DArrayLite data_offset, const int channels,
    const int height, const int width, const int ksize_h,
    const int ksize_w, const int pad_h, const int pad_w,
    const int stride_h, const int stride_w, const int dilation_h, const int dilation_w,
    const int parallel_imgs, const int deformable_group, DArrayLite grad_im,
    cudaStream_t stream) {
    ...
    PARROTS_DISPATCH_FLOATING_TYPES_AND_HALF(data_col.elemType().prim(), ([&] {
        deformable_col2im_gpu_kernel<<<GET_BLOCKS(num_kernels), THREADS_PER_BLOCK, 0, stream>>>(
            num_kernels, data_col.ptr<scalar_t>(), data_offset.ptr<scalar_t>(), channels, height, width, ksize_h,
            ksize_w, pad_h, pad_w, stride_h, stride_w,
            dilation_h, dilation_w, channel_per_deformable_group,
            parallel_imgs, deformable_group, height_col, width_col, grad_im.ptr<scalar_t>());
        }));
...
void deformable_col2im_coord(
    ...
    const int dilation_w, const int parallel_imgs, const int deformable_group,
    DArrayLite grad_offset, cudaStream_t stream) {
    ...
    PARROTS_DISPATCH_FLOATING_TYPES_AND_HALF(data_col.elemType().prim(), ([&] {
        deformable_col2im_coord_gpu_kernel<<<GET_BLOCKS(num_kernels), THREADS_PER_BLOCK, 0, stream>>>(
            num_kernels, data_col.ptr<scalar_t>(), data_im.ptr<scalar_t>(), data_offset.ptr<scalar_t>(),
            channels, height, width, ksize_h, ksize_w, pad_h, pad_w, stride_h, stride_w,
            dilation_h, dilation_w, channel_per_deformable_group,
...
void deform_conv_shape_check(
    DArrayLite input, DArrayLite offset, DArrayLite *gradOutput,
    DArrayLite weight, int kH, int kW, int dH, int dW, int padH,
    ...
    PARROTS_CHECKARGS(weight.isContiguous()) << "weight tensor has to be contiguous";
    ...
    PARROTS_CHECKARGS(weight.dim(2) == kH && weight.dim(3) == kW) <<
         "kernel size should be consistent with weight, but got kH: " << 
         kH << " kW: " << kW << " weight.dim(2): " <<
         weight.dim(2) << ", weight.dim(3): " << weight.dim(3);
    ...
    PARROTS_CHECKARGS(dilationW > 0 && dilationH > 0) <<
         "dilation should be greater than 0, but got dilationH: " <<
    ...
    size_t outputHeight = (inputHeight + 2 * padH - (dilationH * (kH - 1) + 1)) / dH + 1;
    size_t outputWidth = (inputWidth + 2 * padW - (dilationW * (kW - 1) + 1)) / dW + 1;
  
    PARROTS_CHECKARGS(nInputPlane % deformable_group == 0) <<
        "input channels must divide deformable group size";
  
    PARROTS_CHECKARGS(outputWidth >= 1 || outputHeight >= 1) <<
        "Given input size: (" << nInputPlane << " x " << inputHeight <<
        " x " << inputWidth << "). Calculated output size: (" <<
        nOutputPlane << " x " << outputHeight << " x " << outputWidth <<
        "). Output size is too small";
  
    PARROTS_CHECKARGS(input.dim(1) == nInputPlane) <<
        "invalid number of input planes, expected: " << nInputPlane << ", but got: " << input.dim(1);
  
    PARROTS_CHECKARGS(inputHeight >= kH && inputWidth >= kW) <<
        "input image is smaller than kernel";
  
    PARROTS_CHECKARGS(offset.dim(2) == outputHeight && offset.dim(3) == outputWidth) <<
        "invalid spatial dim of offset, expected height: " << outputHeight << " width: " <<
        outputWidth << ", but got height: " << offset.dim(2) << " width: " << offset.dim(3);
    ...
    if (gradOutput != NULL) {
        PARROTS_CHECKARGS(gradOutput->dim(dimf) == nOutputPlane) <<
            ...
            nOutputPlane << ", but got: " << gradOutput->dim(dimf);
        ...
        PARROTS_CHECKARGS(gradOutput->dim(dimh) == outputHeight &&
            ...
            " width: " << outputWidth << " , but got height: " <<
            gradOutput->dim(dimh) << " width: " << gradOutput->dim(dimw);
    }
}

int DeformConvForwardCUDAKernelLauncher(
    ...
    PARROTS_CHECKARGS(offset.dim(0) == batchSize) << "invalid batch size of offset";
    ...
    columns = ctx.createDArrayLite(input.elemType(),
        DArrayShape(nInputPlane * kW * kH, im2col_step * outputHeight * outputWidth));
    columns.setZeros(ctx.getStream());
  
    if (ones.ndims() != 2 ||
        ones.dim(0) * ones.dim(1) < outputHeight * outputWidth) {
        ones = ctx.createDArrayLite(input.elemType(), DArrayShape(outputHeight, outputWidth));
    ...
    auto output_buffer = ctx.createDArrayLite(input.elemType(),
                                              DArrayShape(batchSize / im2col_step, nOutputPlane,
    ...
    for (size_t elt = 0; elt < batchSize / im2col_step; elt++) {
        ...
        weight = weight.view({group, nOutputPlane / group, nInputPlane / group * kH * kW});
  
        for (size_t g = 0; g < group; g++) {
            auto output_g = output_buffer[elt][g];
            gemm(ctx, 1, false, weight[g], false, columns[g], 1, output_g);
        }
    }
    ...
    output_buffer = output_buffer.view({batchSize / im2col_step, nOutputPlane,
                                        im2col_step, outputHeight, outputWidth});
    transpose(ctx, output_buffer, std::vector<size_t>{0, 2, 1, 3, 4}, output_buffer);
...
int DeformConvBackwardInputCUDAKernelLauncher(
    DArrayLite input, DArrayLite offset, DArrayLite gradOutput, DArrayLite gradInput,
    DArrayLite gradOffset, DArrayLite weight, DArrayLite columns,
    ...
    transpose(ctx, gradOutput, std::vector<size_t>{0, 2, 1, 3, 4}, gradOutput);
    ...
    for (size_t elt = 0; elt < batchSize  / im2col_step; elt++) {
        ...
        columns =
            columns.view({columns.dim(0) * columns.dim(1), columns.dim(2)});
        gradOutput = gradOutput.view(
            {gradOutput.dim(0), gradOutput.dim(1) * gradOutput.dim(2),
             im2col_step, outputHeight, outputWidth});
    ...
    if (batch == 0) {
        ...
        offset = offset.view({offset.dim(1), offset.dim(2), offset.dim(3)});
        gradOffset =
            gradOffset.view({offset.dim(1), offset.dim(2), offset.dim(3)});
    }
...
int DeformConvBackwardParametersCUDAKernelLauncher(
    DArrayLite input, DArrayLite offset, DArrayLite gradOutput, DArrayLite gradWeight,
    DArrayLite columns, DArrayLite ones, int kW, int kH, int dW, int dH,
    int padW, int padH, int dilationW, int dilationH, int group,
    ...
    for (size_t elt = 0; elt < batchSize / im2col_step; elt++) {
        ...
        columns = columns.view({group, columns.dim(0) / group, columns.dim(1)});
        gradWeight =
            gradWeight.view({group, gradWeight.dim(0) / group, gradWeight.dim(1) *
                             gradWeight.dim(2) *  gradWeight.dim(3)});
  
        for (int g = 0; g < group; g++) {
            auto gradWeight_g= gradWeight[g];
            gemm(ctx, scale, false, gradOutputBuffer[elt][g], true, columns[g], 1, gradWeight_g);
        }
        gradOutputBuffer = gradOutputBuffer.view(
        ...
        columns =
            columns.view({columns.dim(0) * columns.dim(1), columns.dim(2)});
        gradWeight = gradWeight.view({gradWeight.dim(0) * gradWeight.dim(1),
                                      nInputPlane / group, kH, kW});
    }
