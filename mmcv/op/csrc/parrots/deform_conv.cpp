...
int DeformConvForwardCUDAKernelLauncher(
    ...
    DArrayLite output, DArrayLite columns, DArrayLite ones,
...
int DeformConvBackwardInputCUDAKernelLauncher(
    const DArrayLite input, const DArrayLite offset, const DArrayLite gradOutput,
    DArrayLite gradInput, DArrayLite gradOffset, DArrayLite weight, DArrayLite columns,
    int kW, int kH, int dW, int dH, int padW, int padH, int dilationW, int dilationH,
    int group, int deformable_group, int im2col_step,
    CudaContext& ctx, cudaStream_t stream);
...
int DeformConvBackwardParametersCUDAKernelLauncher(
    const DArrayLite input, const DArrayLite offset, const DArrayLite gradOutput,
    DArrayLite gradWeight, DArrayLite columns, DArrayLite ones,
    int kW, int kH, int dW, int dH, int padW, int padH, int dilationW, int dilationH,
...
void deform_conv_forward_cuda(
    ...
    int kW, kH, dW, dH, padW, padH, dilationW, dilationH, group, deformable_group, im2col_step;
    SSAttrs(attr)
        .get<int>("kW", kW)
        .get<int>("kH", kH)
        .get<int>("dW", dW)
    ...
    DeformConvForwardCUDAKernelLauncher(
        ...
        group, deformable_group, im2col_step, ctx, stream);
}
...
void deform_conv_backward_parameters_cuda(
    ...
    DeformConvBackwardParametersCUDAKernelLauncher(
        ...
        kW, kH, dW, dH, padW, padH, dilationW, dilationH,
        group, deformable_group, scale, im2col_step, ctx, stream);
}
