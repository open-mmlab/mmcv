...
int DeformRoIPoolForwardCUDAKernelLauncher(
    const DArrayLite input, const DArrayLite rois, const DArrayLite offset, DArrayLite output,
...
void deform_roi_pool_forward_cuda(
    CudaContext& ctx,
...
void deform_roi_pool_backward_cuda(
    CudaContext& ctx,
    const SSElement& attr,
    const OperatorBase::in_list_t& ins,
    OperatorBase::out_list_t& outs) {
    ...
    cudaStream_t stream = getStreamNative<CudaDevice>(ctx.getStream());
    DeformRoIPoolBackwardCUDAKernelLauncher(grad_output, input, rois, offset,
