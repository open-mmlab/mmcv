...
void modulated_deformable_im2col_cuda(
    ...
    const int deformable_group, DArrayLite data_col, cudaStream_t stream) {
    ...
    PARROTS_DISPATCH_FLOATING_TYPES_AND_HALF(data_im.elemType().prim(), ([&] {
        modulated_deformable_im2col_gpu_kernel<<<GET_BLOCKS(num_kernels), THREADS_PER_BLOCK, 0, stream>>>(
...
void modulated_deformable_col2im_cuda(
    ...
    const int pad_h, const int pad_w, const int stride_h, const int stride_w,
    const int dilation_h, const int dilation_w,
    const int deformable_group, DArrayLite grad_im, cudaStream_t stream) {

    const int channel_per_deformable_group = channels / deformable_group;
    const int num_kernels = channels * kernel_h * kernel_w * batch_size * height_col * width_col;

    PARROTS_DISPATCH_FLOATING_TYPES_AND_HALF(data_col.elemType().prim(), ([&] {
        modulated_deformable_col2im_gpu_kernel<<<GET_BLOCKS(num_kernels), THREADS_PER_BLOCK, 0, stream>>>(
...
void modulated_deformable_col2im_coord_cuda(
    ...
    DArrayLite grad_offset, DArrayLite grad_mask, cudaStream_t stream) {

    const int num_kernels = batch_size * height_col * width_col * 2 * kernel_h * kernel_w * deformable_group;
    const int channel_per_deformable_group = channels * kernel_h * kernel_w / deformable_group;

    PARROTS_DISPATCH_FLOATING_TYPES_AND_HALF(data_col.elemType().prim(), ([&] {
        modulated_deformable_col2im_coord_gpu_kernel<<<GET_BLOCKS(num_kernels), THREADS_PER_BLOCK, 0, stream>>>(
...
void ModulatedDeformConvForwardCUDAKernelLauncher(
    ...
    PARROTS_CHECKARGS(kernel_h_ == kernel_h && kernel_w_ == kernel_w) <<
        "Input shape and kernel shape wont match: (" << kernel_h <<
        " x " << kernel_w << " vs " << kernel_h_ << " x " <<
        kernel_w_ << ").";
    ...
    for (size_t b = 0; b < batch; b++) {
        ...
        for (size_t g = 0; g < group; g++) {
            ...
            gemm(ctx, 1, false, weight[g].view({weight.dim(1), weight.dim(2) * weight.dim(3) * weight.dim(4)}),
...
void ModulatedDeformConvBackwardCUDAKernelLauncher(
    ...
    for (size_t b = 0; b < batch; b++) {
        ...
        for (size_t g = 0; g < group; g++) {
            ...
            gemm(ctx, 1, true, weight[g].view({weight.dim(1), weight.dim(2) * weight.dim(3) * weight.dim(4)}),
        ...
        for (size_t g = 0; g < group; g++) {
            auto grad_weight_g = grad_weight[g].view({grad_weight.dim(1), grad_weight.dim(2) * grad_weight.dim(3) * grad_weight.dim(4)});
            gemm(ctx, 1, false, grad_output[b][g].view({grad_output.dim(2), grad_output.dim(3) * grad_output.dim(4)}),
                 true, columns[g], 1, grad_weight_g);

            if (with_bias) {
                auto grad_bias_g = grad_bias[g].view({grad_bias.dim(1), 1});
                gemm(ctx, 1, false, grad_bias[b][g].view({grad_bias.dim(2), grad_bias.dim(3) * grad_bias.dim(4)}),
