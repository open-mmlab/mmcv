...
int ROIPoolForwardCUDAKernelLauncher(
    const DArrayLite input, const DArrayLite rois, DArrayLite output, DArrayLite argmax,
    int pooled_height, int pooled_width, float spatial_scale, cudaStream_t stream);

int ROIPoolBackwardCUDAKernelLauncher(
    const DArrayLite grad_output, const DArrayLite rois, const DArrayLite argmax, DArrayLite grad_input,
    int pooled_height, int pooled_width, float spatial_scale, cudaStream_t stream);
...
void roi_pool_forward_cuda(
    ...
    ROIPoolForwardCUDAKernelLauncher(
        ...
        pooled_height, pooled_width, spatial_scale, stream);
}

void roi_pool_backward_cuda(
    CudaContext& ctx,
    const SSElement& attr,
    const OperatorBase::in_list_t& ins,
    OperatorBase::out_list_t& outs){
