...
int CARAFEForwardCUDAKernelLauncher(
    const DArrayLite features, const DArrayLite masks,
    DArrayLite rfeatures, DArrayLite routput, DArrayLite rmasks, DArrayLite output,
    ...
    // one warp per pixel
    PARROTS_DISPATCH_FLOATING_TYPES_AND_HALF(features.elemType().prim(), ([&] {
    ...
    PARROTS_DISPATCH_FLOATING_TYPES_AND_HALF(features.elemType().prim(), ([&] {
          ...
          CARAFEForward<scalar_t>
              <<<divideUP(num_kernels, THREADS_PER_BLOCK), THREADS_PER_BLOCK, 0, stream>>>(
                  ...
                  output_height, output_width, mask_channels, routput.ptr<scalar_t>());
        }));
    PARROTS_DISPATCH_FLOATING_TYPES_AND_HALF(features.elemType().prim(), ([&] {
...
int CARAFEBackwardCUDAKernelLauncher(
    const DArrayLite top_grad, const DArrayLite rfeatures, const DArrayLite masks,
    DArrayLite rtop_grad, DArrayLite rbottom_grad_hs, DArrayLite rbottom_grad,
    DArrayLite rmask_grad, DArrayLite bottom_grad, DArrayLite mask_grad,
    const int kernel_size, const int group_size, const int scale_factor,
    cudaStream_t stream) {
    ...
    PARROTS_DISPATCH_FLOATING_TYPES_AND_HALF(top_grad.elemType().prim(), ([&] {
          const int dh = divideUP(channels, kTileDim);
          const int dw = divideUP(output_height * output_width, kTileDim);
          BatchTranspose2DCUDAKernel<scalar_t>
              <<<batch_size * dh * dw, dim3(kTileDim, kBlockRows), 0, stream>>>(
    ...
    PARROTS_DISPATCH_FLOATING_TYPES_AND_HALF(top_grad.elemType().prim(), ([&] {
          ...
          CARAFEBackward_Feature<scalar_t>
              <<<divideUP(num_kernels, THREADS_PER_BLOCK), THREADS_PER_BLOCK, 0, stream>>>(
                  num_kernels, rtop_grad.ptr<scalar_t>(), masks.ptr<scalar_t>(),
                  kernel_size, group_size,
                  scale_factor, channels, input_height, input_width,
                  output_height, output_width, mask_channels, rbottom_grad_hs.ptr<scalar_t>());
        }));
    PARROTS_DISPATCH_FLOATING_TYPES_AND_HALF(top_grad.elemType().prim(), ([&] {
          const int num_kernels =
          ...
          FeatureSum<scalar_t>
              <<<divideUP(num_kernels, THREADS_PER_BLOCK), THREADS_PER_BLOCK, 0, stream>>>(
                  num_kernels, rbottom_grad_hs.ptr<scalar_t>(), scale_factor, channels,
                  input_height, input_width, rbottom_grad.ptr<scalar_t>());
        }));
    PARROTS_DISPATCH_FLOATING_TYPES_AND_HALF(top_grad.elemType().prim(), ([&] {
    ...
    PARROTS_DISPATCH_FLOATING_TYPES_AND_HALF(top_grad.elemType().prim(), ([&] {
          ...
          CARAFEBackward_Mask<scalar_t>
              <<<divideUP(num_kernels, THREADS_PER_BLOCK), THREADS_PER_BLOCK, 0, stream>>>(
                  num_kernels, rtop_grad.ptr<scalar_t>(), rfeatures.ptr<scalar_t>(),
                  kernel_size, group_size,
                  scale_factor, channels, input_height, input_width,
                  output_height, output_width, mask_channels, rmask_grad.ptr<scalar_t>());
        }));
    PARROTS_DISPATCH_FLOATING_TYPES_AND_HALF(top_grad.elemType().prim(), ([&] {
          const int dh = divideUP(output_height * output_width, kTileDim);
