...
int MaskedIm2colForwardCUDAKernelLauncher(
    ...
    const DArrayLite mask_w_idx, DArrayLite top_data,
    ...
    PARROTS_DISPATCH_FLOATING_TYPES_AND_HALF(bottom_data.elemType().prim(), ([&] {
          MaskedIm2colForward<scalar_t>
              <<<GET_BLOCKS(output_size), THREADS_PER_BLOCK, 0, stream>>>(
                  output_size, bottom_data.ptr<scalar_t>(),
                  height, width, kernel_h, kernel_w, pad_h, pad_w,
                  mask_h_idx.ptr<int64_t>(), mask_w_idx.ptr<int64_t>(),
