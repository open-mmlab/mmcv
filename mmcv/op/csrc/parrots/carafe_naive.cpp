...
int CARAFENAIVEBackwardCUDAKernelLauncher(
    const DArrayLite top_grad, const DArrayLite features, const DArrayLite masks,
    DArrayLite bottom_grad, DArrayLite mask_grad,
    const int kernel_size, const int group_size, const int scale_factor,
    cudaStream_t stream);

void carafe_naive_forward_cuda(
    CudaContext& ctx,
    ...
    CARAFENAIVEForwardCUDAKernelLauncher(
        features, masks, output, kernel_size, group_size, scale_factor, stream);
}

void carafe_naive_backward_cuda(
    CudaContext& ctx,
    const SSElement& attr,
    const OperatorBase::in_list_t& ins,
    OperatorBase::out_list_t& outs) {
