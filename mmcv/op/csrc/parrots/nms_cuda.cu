...
DArrayLite NMSCUDALauncher(
    const DArrayLite boxes_sorted,
    const DArrayLite order,
    const DArrayLite areas,
    float iou_threshold,
    int offset,
    CudaContext& ctx,
    cudaStream_t stream) {
    ...
    auto mask = ctx.createDArrayLite(DArraySpec::array(Prim::Int64, DArrayShape(boxes_num, col_blocks)));
    ...
    auto keep_t = ctx.createDArrayLite(DArraySpec::array(Prim::Uint8, boxes_num), getHostProxy());
