...
__device__ inline bool devIoU(float const * const a, float const * const b, const int offset, const float threshold) {
    ...
    float width = fmaxf(right - left + offset, 0.f), height = fmaxf(bottom - top + offset, 0.f);
...
__global__ void nms_cuda(const int n_boxes, const float iou_threshold, const int offset,
