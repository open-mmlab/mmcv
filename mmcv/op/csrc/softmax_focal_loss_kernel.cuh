...
__global__ void softmax_focal_loss_forward_cuda_kernel(
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    if (label >= 0) {
        output[index] = -alpha * pow((T)1. - pred, gamma) * log(max(pred, (T)FLT_MIN));
    } else {
        output[index] = 0;
    }
    if (weight != NULL) {
...
__global__ void softmax_focal_loss_backward_cuda1_kernel(
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    if (label >= 0) {
        buff[index] = alpha * (-pow((T)1. - pred, gamma) + gamma * pow((T)1. - pred, gamma - 1) * pred * log(max(pred, (T)FLT_MIN)));
    } else {
        buff[index] = 0;
    }
    if (weight != NULL) {
