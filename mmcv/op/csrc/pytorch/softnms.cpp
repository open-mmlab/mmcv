...
Tensor SoftNMSCPULauncher(Tensor boxes, Tensor scores, Tensor dets, float iou_threshold, float sigma, float min_score, int method, int offset) {
    if (boxes.numel() == 0) {
...
Tensor softnms(Tensor boxes, Tensor scores, Tensor dets, float iou_threshold, float sigma, float min_score, int method, int offset) {
    if (boxes.device().is_cuda()) {
        AT_ERROR("No support for gpu softnms!");
    } else {
        return SoftNMSCPULauncher(boxes, scores, dets,
                                  iou_threshold, sigma, min_score,
                                  method, offset);
    }
}
