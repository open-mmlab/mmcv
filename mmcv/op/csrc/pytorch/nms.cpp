...
Tensor NMSCUDALauncher(Tensor boxes, Tensor scores, float iou_threshold, int offset);

Tensor NMSCPULauncher(Tensor boxes, Tensor scores, float iou_threshold, int offset) {
...
Tensor nms(
    ...
    if (boxes.device().is_cuda()) {
        ...
        return NMSCUDALauncher(boxes, scores, iou_threshold, offset);
    } else {
        CHECK_CPU_INPUT(boxes);
        CHECK_CPU_INPUT(scores);
        return NMSCPULauncher(boxes, scores, iou_threshold, offset);
    }
}
