...
class WrappedNet(nn.Module):
    ...
    Args:
        ...
        hooked_features_cpu (dict): Specify the features to be
            recorded.
    ...
    def __init__(
        ...
        self.hooked_features_cpu = hooked_features_cpu
        ...
        for idx, (name, module) in enumerate(model.named_modules()):
            if name in modules_to_record or idx in modules_to_record:
                features_hook = self.get_input_output_hook(
                    name, idx, self.hooked_features_cpu)
    ...
    def forward(self, inputs_tuple):
        ...
        for name in self.hooked_features_cpu:
            outputs[name] = self.hooked_features_cpu[name]
...
class PoplarExecutorForMMCV(PoplarExecutor):
    ...
    def run_model(self, data_dict):
        ...
        for name in output_dict:
            if name in self.hooked_features_cpu:
...
class TrainEvalModel:
    ...
    def train_step(self, data, optimizer=None, **kwargs):
        ...
        if (self._train_executor.isCompiled() and
            ...
            hooked_features_cpu = {**(self._train_executor.hooked_features_cpu)}
        # run in IPU mode
