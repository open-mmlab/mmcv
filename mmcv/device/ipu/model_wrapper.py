...
from .utils import (cfg2options, compare_ndarray, model_sharding,
...
class WrappedNet(nn.Module):
    ...
    Args:
        ...
        inter_outputs_in_cpu (dict): Specify the features to be
            recorded.
        img_norm_cfg (Dict[str, list]): std and mean for normalizing image on
            IPU side.
        ...
        fp16 (bool): The dtype of all tensors in the forward function of this
            class will be invalid, so an additional flag is required to
            identify the precision.
    """
    ...
    def normalize(self, img):
        if self.fp16:
            img = img.half()
            mean = torch.Tensor(self.img_norm_cfg['mean']).half().reshape(
                3, 1, 1)
            std = torch.Tensor(self.img_norm_cfg['std']).half().reshape(
                3, 1, 1)
            img = (img - mean) / std
        else:
            img = img.float()
    ...
    def forward(self, inputs_tuple):
        ...
        if self.img_norm_cfg is not None:
            if isinstance(kwargs['img'], list):
                kwargs['img'] = [self.normalize(im) for im in kwargs['img']]
            else:
                kwargs['img'] = self.normalize(kwargs['img'])
        if self.training:
            if kwargs['img_metas'] is not None:
                split_num = len(kwargs['img'])
                kwargs['img_metas'] = kwargs['img_metas'][:split_num]
        ...
        # batch_input_shape is generated by model, remove it
        if 'img_metas' in kwargs:
            if self.training:
                for img_metas in kwargs['img_metas']:
                    img_metas.pop('batch_input_shape')
            else:
                for img_metas in kwargs['img_metas'][0]:
                    img_metas.pop('batch_input_shape')
        ...
        if isinstance(outputs, torch.Tensor):
            ...
            outputs = {'output of WrappedNet: a list': outputs}
...
class MMPoplarExecutor(PoplarExecutor):
    ...
    def __init__(self,
        ...
        self.ignore_img_metas = ignore_img_metas
    ...
    def run_model(self, data_dict):
        ...
        if 'output of WrappedNet: single tensor' in output_dict:
            ...
            torch_model_output = \
                torch_model_output['output of WrappedNet: single tensor']
        elif 'output of WrappedNet: a list' in output_dict:
            assert len(torch_model_output) == 1
            assert isinstance(
                torch_model_output['output of WrappedNet: a list'], list)
            torch_model_output = \
                torch_model_output['output of WrappedNet: a list']
...
class TrainEvalModel:
    ...
    def post_process(self, results):
        return self.model.model.post_process(results)
...
def ipu_model_wrapper(model,
    ...
    Args:
        ...
        modules_to_record (mmcv.Config, list): Index or name of modules which
            will be recorded for output. It is necessary to specify output for
            static graph of model training or inference.
        ipu_model_cfg (dict): A dictionary contains split_edges and
    ...
    model = model_sharding(model, ipu_model_cfg.get('split_edges', []))

    # set model partition
    if optimizer is None:
        train_model = None
    else:
        train_model = copy.copy(model).train()
