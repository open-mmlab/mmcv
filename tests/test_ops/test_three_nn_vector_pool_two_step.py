...
def test_three_nn(device):
    xyz = torch.Tensor([[0.7911, 4.1821, 18.1309], [9.8552, 19.9272, 7.4532],
                        [17.0715, 9.8851, 5.8078], [4.3750, 1.1232, 18.0702],
                        [14.0227, 9.5781, 15.7914], [3.0038, 8.7471, 12.6253],
                        [17.1353, 13.0427,
                         13.4723], [1.4284, 12.0409, 16.0280],
                        [10.5802, 11.9821,
                         10.6400], [11.2924, 16.3918, 16.3261],
                        ...
                        [12.7604, 10.6771, 19.1813],
                        [10.3219, 10.4839, 14.7624]]).cuda()
    new_xyz = torch.Tensor([[0.1411, 15.6141, 9.3022],
                            [15.6595, 0.9505, 19.3470],
                            [8.0824, 10.3586,
                             17.3501], [7.3926, 9.9670, 6.6586],
                            [13.8781, 8.9048,
                             5.8824], [11.1121, 0.0274, 9.4883],
                            [0.4287, 1.5586,
                             6.9646], [2.7858, 1.8852, 15.0609],
                            [6.0411, 2.8716, 18.9102],
                            [9.1480, 10.8151, 17.0509],
                            [5.1243, 8.9133, 18.5356],
                            [19.7255, 14.6383, 9.3120]]).cuda()
    ...
    xyz_batch_cnt = torch.tensor([8, 12]).cuda().int()
    new_xyz_batch_cnt = torch.tensor([4, 8]).cuda().int()
