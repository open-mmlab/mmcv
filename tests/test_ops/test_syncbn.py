...
class TestSyncBN(object):
    ...
    def _test_syncbn_train(self, size=1, half=False, empty=False):
        ...
        syncbn = SyncBatchNorm(
            3, group=group, stats_mode=('N' if empty else 'default')).cuda()
    ...
    def test_syncbn(self, size, half, empty):
        self._test_syncbn_train(size=size, half=half, empty=empty)
