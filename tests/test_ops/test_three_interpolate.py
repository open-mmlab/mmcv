...
from mmcv.ops import stack_three_interpolate, three_interpolate
...
@pytest.mark.skipif(
    not torch.cuda.is_available(), reason='requires CUDA support')
def test_stack_three_interpolate():
    features = torch.tensor([[2.4350, 4.7516, 4.4995, 2.4350, 2.4350, 2.4350],
                             [3.1236, 2.6278, 3.0447, 3.1236, 3.1236, 3.1236],
                             [2.6732, 2.8677, 2.6436, 2.6732, 2.6732, 2.6732],
                             [0.0124, 7.0150, 7.0199, 0.0124, 0.0124, 0.0124],
    ...
    idx = torch.tensor([[0, 1, 2], [2, 3, 4], [2, 3, 4], [0, 1, 2], [0, 1, 2],
                        [0, 1, 3], [0, 2, 3], [1, 3, 4], [2, 1, 4], [0, 2, 4],
                        [0, 2, 4], [0, 1, 2]]).int().cuda()
    ...
    output = stack_three_interpolate(features, idx, weight)
    expected_output = torch.tensor(
        [[2.7439, 3.4157, 3.3959, 2.7439, 2.7439, 2.7439],
         [2.6732, 2.8677, 2.6436, 2.6732, 2.6732, 2.6732],
         [2.6732, 2.8677, 2.6436, 2.6732, 2.6732, 2.6732],
         [2.7439, 3.4157, 3.3959, 2.7439, 2.7439, 2.7439],
         [2.7439, 3.4157, 3.3959, 2.7439, 2.7439, 2.7439],
         [1.8570, 4.7981, 4.8547, 1.8570, 1.8570, 1.8570],
         [1.7068, 4.8781, 4.7210, 1.7068, 1.7068, 1.7068],
         [3.1236, 2.6278, 3.0447, 3.1236, 3.1236, 3.1236],
         [2.6732, 2.8677, 2.6436, 2.6732, 2.6732, 2.6732],
         [1.8096, 2.5397, 2.4947, 1.8096, 1.8096, 1.8096],
         [1.8096, 2.5397, 2.4947, 1.8096, 1.8096, 1.8096],
         [2.7439, 3.4157, 3.3959, 2.7439, 2.7439, 2.7439]],
        device='cuda:0')
    assert torch.allclose(output, expected_output, 1e-4)
